...
namespace unispace
{
	...
	public:
		 ...
		 * @param	part1	第二个64位数.
		 *************************************************************************/
		us_uuid(uint64_t part1, uint64_t part2)
			:m_part_1(part1), m_part_2(part2)
		{
		}
		 ...
		 * @author	yimin.liu@unispace-x.com
		 * @date	2016/3/23
		 * @param	other	用于比较的另一个uuid.
		 * @return	相等返回true，不等返回false.
		 *************************************************************************/
		bool operator ==(const us_uuid& other)const;
		...
		/**********************************************************************//**
		 * @brief	判断UUID是否由全零值构成{00000000-0000-0000-0000-000000000000}.
		 * @author	yimin.liu@unispace-x.com
		 * @date	2018/8/10
		 * @return	全零值构成返回true.
		 *************************************************************************/
		bool is_null() const;
		inline bool is_zero() const { return is_null(); }
...
{
	template <>
	struct hash<unispace::us_uuid>
	{
		inline std::size_t operator()(const unispace::us_uuid &key) const
		{
			std::pair<uint64_t, uint64_t>& rkey = (std::pair<uint64_t, uint64_t>&)key;
			const uint64_t kMul = 0x9ddfea08eb382d69ULL;
			uint64_t a = (rkey.first) ^ (rkey.second) * kMul;
			a ^= (a >> 47);
			uint64_t b = ((rkey.second) ^ a) * kMul;
			b ^= (b >> 47);
			b *= kMul;
			return b;
		}
