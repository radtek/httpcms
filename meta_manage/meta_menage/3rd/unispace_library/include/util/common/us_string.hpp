...
namespace unispace
{
	...
	public:
		 ...
		 * @return	替换子串后的字符串.
		 *************************************************************************/
		_US_UTIL_DLL us_ustring& replace_substr(char oldchar, char newchar);
	...
	public:
		/**********************************************************************//**
		 * @brief	从数值构建字符串.
		 *			非浮点数部分直接采用的std::to_string做的转换，浮点数部分使用
		 *			grisu2_dtoa做的转换.
		 * @author	yimin.liu@unispace-x.com
		 * @date	2017/9/22
		 * @param	num	数值.
		 ...
		 *************************************************************************/
		_US_UTIL_DLL static us_ustring from_num(short int num);
		_US_UTIL_DLL static us_ustring from_num(unsigned short num);
		_US_UTIL_DLL static us_ustring from_num(int num);
		_US_UTIL_DLL static us_ustring from_num(unsigned int num);
		_US_UTIL_DLL static us_ustring from_num(long long int num);
		_US_UTIL_DLL static us_ustring from_num(unsigned long long num);
		_US_UTIL_DLL static us_ustring from_num(float num);
		_US_UTIL_DLL static us_ustring from_num(double num);

		/**********************************************************************//**
		 * @brief	无损转换double数值到字符串.
		 *			因为std::to_string中的转换并非完全正确的.
		 * @author	yimin.liu@unispace-x.com
		 * @date	2017/9/22
		 * @param	num	要转换的double数.
		 * @return	以UTF8编码的us_string字符串.
		 *************************************************************************/
		_US_UTIL_DLL static us_ustring grisu2_dtoa(double num);
