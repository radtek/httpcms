...
namespace unispace
{
	...
	namespace filesystem
	{
		...
		/**********************************************************************//**
		 * @brief	返回用户的临时目录的绝对路径.
		 *			在Windows NT/2000下，这个函数返回TMP环境变量的值。没有则返回
		 *			TEMP环境变量的值。还没有则返回USERPROFILE的值<br>
		 *			类Unix操作系统，返回"/tmp/"路径。<br>
		 * @author	yimin.liu@unispace-x.com
		 * @date	2017/9/26
		 * @return	用户的临时目录的绝对路径.
		 *************************************************************************/
		_US_UTIL_DLL us_ustring temp_dir_path();
		...
		/**********************************************************************//**
		 * @brief	获取路径中的目录部分.
		 *			除了basebane之外的部分，例如"/fo/bar /bz .txt"则返回"/fo/bar/"。
		 * @author	yimin.liu@unispace-x.com
		 * @date	2018/7/27
		 * @param	filepath		文件路径.
		 * @return	目录路径.
		 *************************************************************************/
		_US_UTIL_DLL us_ustring dirname(const us_ustring& filepath);

		/**********************************************************************//**
		 * @brief	获取文件后缀名.
		 *			如果文件名或后缀名不存在，则返回空字符串.
		 *			例如"/fo/.txt"和".txt"或"/fs/23/"都返回空字符串.
		 * @author	yimin.liu@unispace-x.com
		 * @date	2018/7/27
		 * @param	filepath		文件名称或路径.
		 * @return	文件后缀名(不含'.').
		 *************************************************************************/
		_US_UTIL_DLL us_ustring file_extension(const us_ustring& filepath);

		/**********************************************************************//**
		 * @brief	获取精简描述的路径形式.
		 *			将路径中的'.'和'../../'这样的层级描述去除，获得简短路径.
		 * @author	yimin.liu@unispace-x.com
		 * @date	2018/7/27
		 * @param	path		文件或目录路径.
		 * @return	精简后的路径.
		 *************************************************************************/
		_US_UTIL_DLL us_ustring path_prune(const us_ustring& path);
	}
};
