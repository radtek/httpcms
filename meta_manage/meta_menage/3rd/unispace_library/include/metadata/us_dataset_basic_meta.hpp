...
namespace unispace
{
	...
	public:
		 ...
		 *************************************************************************/
		explicit us_dataset_basic_meta(
			const us_ustring& name = us_ustring(),bool create_uuid = false);
	...
	public:
		...
		int64_t		m_all_file_size;///< 所有影像总的文件大小
		int32_t		m_min_level;	///< 可显示最小网格层级
		int32_t		m_max_level;	///< 可显示最大网格层级
		int32_t		m_rgb_band[3];	///< 用于取瓦片定为RGB三个波段的波段序号
		...
		uint32_t	m_band_count;	///< 波段数
	};

	/**********************************************************************//**
	 * @class	us_raster_dataset_meta
	 * @brief	栅格数据集基本元数据.
	 * @author	yimin.liu@unispace-x.com
	 * @date	2018/8/2
	 *************************************************************************/
	class us_raster_dataset_meta {
	public:
		us_ustring	m_url;			///< 数据集URL
		us_ustring	m_desc;			///< 数据集描述信息
		us_uuid		m_sprid;		///< 空间参考ID
		int64_t		m_ctime;		///< 入库时间
		int64_t		m_mtime;		///< 最近修改时间
		int64_t		m_dtime;		///< 数据时间
		int64_t		m_all_file_size;///< 所有影像总的文件大小
		...
		uint32_t	m_band_count;	///< 波段数
		int32_t		m_has_hot;		///< 冷热标识
		double		m_res_h_max;	///< 分辨率横向最大值
		double		m_res_v_max;	///< 分辨率纵向最大值
		double		m_res_h_mean;	///< 分辨率横向平均值
		double		m_res_v_mean;	///< 分辨率纵向平均值
		double		m_res_h_min;	///< 分辨率横向最小值
		double		m_res_v_min;	///< 分辨率纵向最小值
		us_geo_rect m_geo_rect;		///< 地理范围
		us_geo_rect m_latlng_rect;	///< 经纬度地理范围
		std::vector<us_ustring> m_imgurls;	///< 包含影像的URL
	};
}
