...
namespace unispace
{
	...
	public:
		 ...
		 *************************************************************************/
		us_image_basic_meta()
			:m_ctime(0), m_dtime(0), m_mtime(0),
			m_file_size(0),
			m_resolution_h(0.0), m_resolution_v(0.0),
			m_pixel_h(0), m_pixel_v(0),
			m_band_count(0), m_type(0)
		{
		}
		 ...
		 * @param	uu	一个uuid对象.
		 *************************************************************************/
		inline bool set_uuid(const us_uuid& uu)
		{
			m_id = uu;
			return true;
		}
		 ...
		 *************************************************************************/
		inline bool set_uuid(const char* uuidstr)
		{
		 ...
		 * @return	The uuid.
		 *************************************************************************/
		inline const us_uuid& get_uuid()const
		{
			return m_id;
		}
		inline const us_ustring& get_virtual_path()const
		{
			return m_link_path.empty() ? m_image_url : m_link_path;
		}
	public:
		...
		time_t		m_dtime;            ///< 数据时间(data time)
		time_t		m_mtime;	        ///< 修改时间(modify time)
		int64_t		m_file_size;		///< 影像文件的大小
		...
		us_ustring	m_link_path;		///< 影像链接路径(URL不一定指向真实影像)
	};
}
