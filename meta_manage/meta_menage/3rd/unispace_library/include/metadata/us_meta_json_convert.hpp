...
namespace unispace
{
	 ...
	 * @param	in_meta		传入数据集元信息.
	 * @param	out_json	传出json字符串.
	 * @param	hasimgurls	是否包含影像的URL.
	 * @return	成功返回true,失败返回false.
	 *************************************************************************/
	_US_METADATA_DLL bool us_dataset_meta_to_json(
		const us_raster_dataset_meta& in_meta,us_ustring* out_json, bool hasimgurls);
	 ...
	 *************************************************************************/
	_US_METADATA_DLL bool us_json_to_dataset_meta(
		const us_ustring& in_json, us_raster_dataset_meta* out_meta, bool hasimgurls);
