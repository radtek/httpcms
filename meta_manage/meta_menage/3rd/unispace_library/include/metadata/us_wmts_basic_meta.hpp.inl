	...
	inline us_ustring us_wmts_basic_meta::get_tile_matrix_set_xml()const
	{
		...
		tms.append("<ows:SupportedCRS>urn:ogc:def:crs:EPSG::").append(us_ustring().set_num(epsg_code)).append("</ows:SupportedCRS>");
		{
			...
			for (int level = m_min_level; level <= m_max_level; ++level) {
				...
				tms.append("<ows:Identifier>").append(std::to_string(level)).append("</ows:Identifier>");
				tms.append("<ScaleDenominator>").append(us_ustring::grisu2_dtoa(scaledenom)).append("</ScaleDenominator>");
				tms.append("<TopLeftCorner>").append(us_ustring::grisu2_dtoa(grt.get_origin().get_y() + grt.get_height()));
				tms.append(" ").append(us_ustring::grisu2_dtoa(grt.get_origin().get_x())).append("</TopLeftCorner>");
				tms.append("<TileWidth>").append(std::to_string(m_tile_pixel_h)).append("</TileWidth>");
				tms.append("<TileHeight>").append(std::to_string(m_tile_pixel_v)).append("</TileHeight>");
				tms.append("<MatrixWidth>").append(std::to_string(ranks.m_width)).append("</MatrixWidth>");
				tms.append("<MatrixHeight>").append(std::to_string(ranks.m_height)).append("</MatrixHeight>");
				tms.append("</TileMatrix>");
			}
		}
