...
namespace unispace
{
	...
	public:
		...
		*************************************************************************/
		...
		{
			us_size<int> maxrank = m_tile_regul.m_rank_base * (0x1 << level);
			double tile_span_x = m_tile_regul.m_rect_base.get_width() / maxrank.m_width;
			double tile_span_y = m_tile_regul.m_rect_base.get_height() / maxrank.m_height;
			double min_x = tile_span_x * col + m_tile_regul.m_rect_base.get_min_x();
			double max_x = min_x + tile_span_x;
			double max_y = tile_span_y * (maxrank.m_height - row) + m_tile_regul.m_rect_base.get_min_y();
			double min_y = max_y - tile_span_y;
		...
		{
			return m_tile_regul.get_tile_matrix_set_xml(
				m_meta.m_layer_name, m_meta.m_spr_uuid,
