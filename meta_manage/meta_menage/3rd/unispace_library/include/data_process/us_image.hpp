...
namespace unispace
{
	...
	public:
		...
		us_image& operator= (const us_image& img);
	...
	public:
		...
		/**********************************************************************//**
		 * @brief	获取实际关联的影像的颜色表.
		 *			注意，返回颜色表的生命周期与调用此接口的对象一致.
		 * @author	yimin.liu@unispace-x.com
		 * @date	2017/8/8
		 * @return	成功返回颜色表地址指针(GDALColorTableH)，失败返回NULL.
		 *************************************************************************/
		void* get_color_table()const;
		 ...
		 * @date	2017/11/07
		 * @param	key		元数据项的键.
		 * @param	domain	元数据项所在的域.
		 * @param   defaultvalue 不存在时候的默认值.
		 * @return	返回获取到的key对应的value.
		 *************************************************************************/
		std::string get_metadata_item(const char *key,const char* domain = "",
		 ...
		 *************************************************************************/
		bool set_metadata_item(const char *key,const char* value, const char* domain = "")const;
	...
	private:
		...
		std::shared_ptr<GDALDataset> pdset;
	};
	...
	/////////////////////////////////////////////////////////////////////////////
	inline int64_t get_image_rgb_embed_alpha_buffer(
		const us_image& image, void * pData, int64_t nBufSize)
	{
		return get_image_rgb_embed_alpha_buffer(
			image, pData, nBufSize, true, 3, 2, 1);
	}
	inline int64_t get_image_rgb_embed_alpha_buffer(
		const us_image& image, void * pData, int64_t nBufSize,
		bool isLikeRGB)
	{
		return get_image_rgb_embed_alpha_buffer(
			image, pData, nBufSize, isLikeRGB, 3, 2, 1);
	}
