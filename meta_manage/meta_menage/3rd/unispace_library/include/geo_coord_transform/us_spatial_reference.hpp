...
namespace unispace
{
	...
	public:
		...
		/**********************************************************************//**
		 * @brief	不同空间参考下坐标转换.
		 *			变换传入矩形外包框从源坐标系统到目标坐标系统。因为矩形外包框的转换
		 *			涉及到变形等问题，所以转换后的结果是四个角点在目标坐标系下的外包框，
		 *			目标外包框应是大于源外包框范围的。<br><b>注意:</b>
		 *			传入的地理位置需要度为单位。并且返回的也是度单位。
		 * @author	yimin.liu@unispace-x.com
		 * @date	2018/8/10
		 * @param	src		源坐标系统.
		 * @param	dst		目标坐标系统.
		 * @param	src_rect		待转换的矩形外包框.
		 * @param	dst_rect		传出转换到目标坐标系的矩形外包框.
		 * @return	成功返回0，失败返回错误代码.
		 *			如果所有转换都失败，那么返回错误代码。如果个别点失败(例如地平线上的点)，转换
		 *			结果将被设置为HUGE_VAL。如果传入的数据是HUGE_VAL，将不做转换。
		 *************************************************************************/
		static int transform(const us_spatial_reference& src,
							 const us_spatial_reference& dst,
							 const us_geo_rect& src_rect , us_geo_rect* dst_rect);
