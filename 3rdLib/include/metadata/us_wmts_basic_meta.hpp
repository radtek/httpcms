...
namespace unispace
{
	struct us_wmts_basic_meta {
		...
		us_ustring	m_desc;			///< wmts图层描述
		int64_t		m_ctime;		///< 创建时间
		int64_t		m_dtime;		///< 数据时间
		int64_t     m_mtime;		///< 修改时间
		us_uuid		m_uuid;			///< 唯一ID
		...
		int32_t		m_max_level;	///< 最大级别
		int32_t		m_min_level;	///< 最小级别
		int32_t		m_tile_pixel_h;	///< 瓦片横向像素数
		int32_t		m_tile_pixel_v;	///< 瓦片纵向像素数
		int32_t		m_tile_type;    ///< 瓦片规则： 0：内部度网格，1：内部米网格；2：谷歌经纬度；3：谷歌Web墨卡托；
		int32_t		m_rgb_band[3];	///< 用于取瓦片定为RGB三个波段的波段序号
		uint32_t	m_band_count;	///< 波段数
        uint32_t    m_dpi_mode;     ///< DPI模式: 0:90.7142857(ArcGIS),1:90.8156652(QGIS),2:96.0(国家标准，天地图)
        uint32_t    m_source_type;  ///< 数据源类型: 0:数据集,1:瓦片包
		us_wmts_basic_meta()
		{
